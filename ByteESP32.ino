...
void setup(void) {
  ...
  u8g2.drawStr(42, 18, "WELCOME");
  u8g2.drawStr(53, 32, "BACK");
  u8g2.drawStr(41, 45, "BYTEASSASSINS");
  ...
  int close_speed = 300; // Time for the eyes to remain CLOSE 

  // Initial square eyes animation
  eye_open();
  delay(open_speed);
  eye_close();
  delay(close_speed);
  ...
  // Transition to circular eyes
  for (int r = 0; r <= 18; r++) {
    draw_circular_eyes(r);
    delay(20); // Adjust delay for smooth transition
  }
  
  // Slowly add a centered smile
  draw_smile();
}
...
void eye_open() {
  ...
  u8g2.drawBox(14, 13, 36, 36); // Left eye
  u8g2.drawBox(78, 13, 36, 36); // Right eye
  u8g2.sendBuffer();
}
...
void eye_close() {
  u8g2.clearBuffer(); // Clear the internal memory
  u8g2.setBitmapMode(1);
  u8g2.drawBox(14, 27, 36, 7); // Left eye (closed)
  u8g2.drawBox(78, 27, 36, 7); // Right eye (closed)
  u8g2.sendBuffer();
}

void draw_circular_eyes(int r) {
  u8g2.clearBuffer();  // Clear the internal memory
  u8g2.setBitmapMode(1);
  u8g2.drawCircle(32, 31, r); // Left eye
  u8g2.drawCircle(96, 31, r); // Right eye
  u8g2.sendBuffer();
}

void draw_smile() {
  // Coordinates for the smile centered between the eyes
  int smile_x_center = 64;  // Center of the screen
  int smile_width = 40;     // Width of the smile
  int smile_y = 63;         // Y-coordinate for the smile

  // Draw the smile arc from left to right, centered
  for (int i = -smile_width / 2; i <= smile_width / 2; i += 4) {
    int x1 = smile_x_center + i;
    int y1 = smile_y - (abs(i) / 6);  // Adjust y1 to curve downwards in the middle for a smile
    int x2 = x1 + 4;
    int y2 = smile_y - (abs(i + 4) / 6); // Similar adjustment to curve the smile

    // Draw the smile segment by segment
  ...
  WiFi.softAP("Byteassassins", "Sumit123");
